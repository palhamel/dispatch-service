...
import { loadAppsConfig } from './config/loader.js'
...
// Load app config
const configPath = new URL('./config/apps.json', import.meta.url).pathname
let appsConfig
try {
  appsConfig = loadAppsConfig(configPath)
} catch (err) {
  console.error('Failed to load apps config:', (err as Error).message)
  console.error('Copy src/config/apps.example.json to src/config/apps.json and configure your apps.')
  process.exit(1)
}
