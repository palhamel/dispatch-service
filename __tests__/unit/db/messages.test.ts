...
describe('MessageStore', () => {
  ...
  describe('getMessages', () => {
    beforeEach(() => {
      // Insert test data
      store.logMessage({ app: 'myapp', channel: 'discord', body: 'Message 1' })
      store.logMessage({ app: 'myapp', channel: 'discord', body: 'Message 2' })
    ...
    it('filters by app', () => {
      const result = store.getMessages({ app: 'myapp' })
      expect(result.messages).toHaveLength(2)
      expect(result.total).toBe(2)
      expect(result.messages.every(m => m.app === 'myapp')).toBe(true)
    })
    ...
    it('filters by both app and status', () => {
      const result = store.getMessages({ app: 'myapp', status: 'failed' })
      expect(result.messages).toHaveLength(1)
  ...
  describe('getStats', () => {
    beforeEach(() => {
      store.logMessage({ app: 'myapp', channel: 'discord', body: 'Msg 1' })
      store.logMessage({ app: 'myapp', channel: 'discord', body: 'Msg 2' })
      store.logMessage({ app: 'myapp', channel: 'discord', body: 'Msg 3' })
    ...
    it('returns per-app statistics', () => {
      ...
      expect(stats.apps['myapp']).toBeDefined()
      expect(stats.apps['myapp'].sent).toBe(2)
      expect(stats.apps['myapp'].failed).toBe(1)
      expect(stats.apps['myapp'].totalMessages).toBe(3)
    })
