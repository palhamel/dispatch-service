...
describe('sendDiscord', () => {
  it('formats message with subject as embed title', async () => {
    ...
    await sendDiscord(channelConfig, payload, 'kodfika')
  ...
  it('includes metadata as embed fields', async () => {
    ...
    const urlField = fields.find((f: any) => f.value === 'https://kodfika.se/about')
  ...
  it('uses app-specific embed color and footer from config', async () => {
    ...
    expect(body.embeds[0].footer.text).toContain('Via Kodfika')
  })
  ...
  it('returns success:true on successful webhook response', async () => {
    ...
    const result = await sendDiscord(channelConfig, payload, 'kodfika')
